...
However, here are some notable changes:

- Tags are now split across multiple slash commands, `/t` to get a tag,
  `/tag-add` to add a tag, and `/tag` for additional tag management commands.
