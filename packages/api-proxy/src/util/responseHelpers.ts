 ...
 */
export async function populateSuccessfulResponse(
  res: ServerResponse,
  data: ResponseLike,
): Promise<void> {
  ...
  if (data.body) {
    await pipeline(
      data.body instanceof Readable ? data.body : Readable.fromWeb(data.body),
      res,
    );
  }
}
 ...
 */
export function populateGeneralErrorResponse(
  res: ServerResponse,
 ...
 * @param error - The error to populate the response with
 */
export function populateRatelimitErrorResponse(
  res: ServerResponse,
 ...
 */
export function populateErrorResponse(
  res: ServerResponse,
  error: unknown,
): boolean {
