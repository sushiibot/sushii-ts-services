...
import { getImageFromReact } from "@sushiibot/html2image";
...
function getNotoFont(): Promise<Buffer> {
  if (notoSans) {
    return Promise.resolve(notoSans);
  }

  return readFile("./fonts/NotoSans-Regular.ttf");
}
...
export async function getUserRank(
  ...
  const rankBuffer = await getImageFromReact(
    <RankCard
      ...
      rep={userData.rep}
      repLevel={repLevel.toString().padStart(2, "0")}
      fishies={userData.fishies}
      isPatron={userData.is_patron}
      patreonEmojiUrl={
      ...
      guild={{
        ...
        currXP: userLevel.nextLevelXpProgress,
        reqXP: userLevel.nextLevelXpRequired,
        xpProgress: userLevel.nextLevelXpPercentage,
      }}
      global={{
        level: globalLevel.level,
        currXP: globalLevel.nextLevelXpProgress,
        reqXP: globalLevel.nextLevelXpRequired,
        xpProgress: globalLevel.nextLevelXpPercentage,
      }}
      ranks={{
        all: {
          rank: guildRanks.all_time.rank,
          total: guildRanks.all_time.total_count,
        },
        week: {
          rank: guildRanks.week.rank,
          total: guildRanks.week.total_count,
        },
        month: {
          rank: guildRanks.month.rank,
          total: guildRanks.month.total_count,
        },
        day: {
          rank: guildRanks.day.rank,
          total: guildRanks.day.total_count,
        },
      }}
    />,
    {
